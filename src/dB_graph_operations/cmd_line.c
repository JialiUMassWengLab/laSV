...
int default_opts(CmdLine * c)
{
  ...
  c->insert_size = 500;
  c->min_sup = 1;
  c->min_contig_len = 100;
  set_string_to_null(c->mode, 30);
...
int parse_cmdline_inner_loop(int argc, char* argv[], int unit_size, CmdLine* cmdline_ptr, char* error_string)
{
  ...
  static struct option long_options[] = {
    ...
    {"insert_size",required_argument,NULL,'v'},
    ...
    {"read_span_file",required_argument, NULL, 'S'},
    {"output_contigs",required_argument, NULL, 'T'},
    {"min_contig_len",required_argument,NULL,'U'},
    {"print_novel_contigs", required_argument, NULL, 'V'},
    {"min_sup", required_argument, NULL, 'W'},
  ...
  opt = getopt_long(argc, argv, "ha:b:c:d:e:f:g:i:jk:l:m:n:o:p:q:r:s:t:u:v:w:xy:z:A:B:C:D:E:F:G:H:I:J:KL:MN:O:P:Q:R:S:T:U:V:W:", long_options, &longopt_index);
  ...
  while ((opt) > 0) {
    ...
    case 'S': //read_span_file
      {
        if (optarg==NULL)
          errx(1,"[--read_span_file] option requires a filename (listing the path to a read span file/q)\n");

        if (strlen(optarg)<MAX_FILENAME_LEN)
          {
            strcpy(cmdline_ptr->read_span_file,optarg);
          }
        else
          {
            errx(1,"[--read_span_file] filename too long [%s]",optarg);
          }

        if (access(optarg,R_OK)==-1){
          errx(1,"[--read_span_file] filename [%s] cannot be accessed",optarg);
        }
        break;
      }
    ...
    case 'T': //output_contigs
      {
        if (optarg==NULL)
          errx(1,"[--output_contigs] option requires a filename (listing the path to a file that contains output contigs/q)\n");

        if (strlen(optarg)<MAX_FILENAME_LEN)
          {
            strcpy(cmdline_ptr->output_contigs,optarg);
          }
        else
          {
            errx(1,"[--output_contigs] filename too long [%s]",optarg);
          }

        if (access(optarg,R_OK)==-1){
          errx(1,"[--output_contigs] filename [%s] cannot be accessed",optarg);
        }
        break;
      }
    ...
    case 'v': //insert size
      {
	if (optarg==NULL)
	  errx(1,"[--insert_size] option requires int argument [insert size]");
	cmdline_ptr->insert_size = atoi(optarg);

	if (cmdline_ptr->insert_size <= 0)
	  errx(1,"[--insert_size] option requires int argument bigger than 0");

	break;
      }

    case 'U': //minimum contig length
      {
	if (optarg==NULL)
	  errx(1,"[--min_contig_len] option requires int argument [minimum contig length]");
	cmdline_ptr->min_contig_len = atoi(optarg);

	if (cmdline_ptr->min_contig_len <= 0)
	  errx(1,"[--min_contig_len] option requires int argument bigger than 0");

	break;
      }

    case 'W': //minimum number of supporting reads
      {
        if (optarg==NULL)
          errx(1,"[--min_sup] option requires int argument [minimum number of supporting reads]");
        cmdline_ptr->min_sup = atoi(optarg);

        if (cmdline_ptr->min_sup < 0)
          errx(1,"[--min_sup] option requires int argument equal or bigger than 0");
    ...
    case 'X': //operation mode
      {
        if (optarg==NULL)
          errx(1,"[--mode] option requires string argument [build OR assembly]");

	strcpy(cmdline_ptr->mode,optarg);

        if (strcmp(cmdline_ptr->mode,"build") != 0 && strcmp(cmdline_ptr->mode,"assembly") != 0)
          errx(1,"[--mode] option has to be either \"build\" OR \"assembly\"");
