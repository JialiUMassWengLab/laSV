...
int main(int argc, char **argv)
{
  ...
  printf("Starting laSV graph operations\n");
...
"Please inform Zam Iqbal (zam@well.ox.ac.uk) so he can fix that UI bug\n");
  ...
  */
  ...
  for(q=0; q<db_graph->number_buckets * db_graph->bucket_size; q++) 
    {
      if ((db_graph->table[q].coverage[0] > 0)&&(!db_node_check_status(&db_graph->table[q],pruned))) {

	all++;
	
	int out_degree = db_node_get_degree(&db_graph->table[q], 0, 0);
	int in_degree = db_node_get_degree(&db_graph->table[q], 0, 1);

	if ((out_degree+in_degree == 1)&&(out_degree*in_degree == 0)) {EndNode++;}
	if ((out_degree == 0)&&(in_degree == 0)) 
	  {
	    IsolateNode++; 
	    db_node_set_status(&db_graph->table[q],pruned);
	  }
	if ((out_degree >= 2)||(in_degree >= 2)) {BranchNode++;}
      }
    }
  printf("\nEnd Nodes:%d\nIsolated Nodes:%d\nBranching Nodes:%d\nAll retained nodes:%d\n\n", EndNode, IsolateNode, BranchNode, all);
  */

  if (strcmp(cmd_line->mode, "build") == 0)
    {
      timestamp();
      printf("Begin to print branch sequences\n\n");
      db_graph_print_all_branches_for_specific_person_or_pop("./branches.fa",db_graph,0);
      timestamp();
      printf("Branches printed\n\n");
    }

  else if (strcmp(cmd_line->mode, "assembly") == 0)
    {
      LinkHashTable* link_list = link_hash_table_new(hash_key_bits-2, (int) bucket_size*0.25, max_retries, kmer_size);
      if (link_list==NULL)
	{
	  die("Giving up - unable to allocate memory for the little hash table\n");
	}
      printf("Little hash table created, number of buckets: %d\n",1 << hash_key_bits);
      
      linkage_build_link_list_from_file(cmd_line->read_span_file, db_graph, link_list);
      timestamp();
      printf("Linkage Table successfully built\n\n");
      
      FILE* output=fopen("contigs.fa","w");
      if (output == NULL)
	{
	  die("Couldn't open file contigs.fa to write!\n\n");
	}
      unsigned int count=0;
      
      timestamp();
      printf("Begin assembly and print contigs\n\n");
      int read_length = cmd_line->max_read_length;
      int insert_len  = cmd_line->insert_size;
      int contig_len  = cmd_line->min_contig_len;
      int minimum_sup = cmd_line->min_sup;
      ...
      for(q=0; q<db_graph->number_buckets * db_graph->bucket_size; q++)
	{
	  if ((db_graph->table[q].coverage[0] > 0)&&(!db_node_check_status(&db_graph->table[q],pruned))&&(!db_node_check_status(&db_graph->table[q],visited)))
		...
		{
		  assemble_contigs_from_a_single_source(&db_graph->table[q], 1, read_length, insert_len, contig_len, minimum_sup, db_graph, link_list, &count, output, 0);
		}
	      else if ((in_degree == 0)&&(out_degree >= 1))
		{
		  assemble_contigs_from_a_single_source(&db_graph->table[q], 0, read_length, insert_len, contig_len, minimum_sup, db_graph, link_list, &count, output, 0);
		}
	    }
	}
		...
		for (x=0; x < 2; x++)
		  {
		    ...
		    for (y=0; y < 4; y++)
		      {
			if (db_node_edge_exist(&db_graph->table[q], y, x, 0))
			  {
			    next = db_graph_get_next_node_for_specific_person_or_pop(&db_graph->table[q],x,&next_orientation,y,&reverse_edge,db_graph,0);
			    if (!db_node_check_status(next, visited))
			      {
				assemble_contigs_from_a_single_source(next, x, read_length, insert_len, contig_len, minimum_sup, db_graph, link_list,&count, output, 0);
				circulate=true;
			      }
			  }
		      }
  ...
  */
  ...
  //  link_hash_table_free(&link_list);
  timestamp();

  if (strcmp(cmd_line->mode, "build") == 0)
    printf("dB_raph build successfully finished\n\n\n");
  else
    printf("dB_graph assembly successfully finished\n\n\n");
