...
open (input, "<$ARGV[1].breakpoints.fa") or die "Can't open $ARGV[1].breakpoints.fa since $!\n";
while (my $line=<input>) {
    chomp($line);
    if ($line =~ /^>/) {
	$line =~ s/\>//;
	...
	system("grep $a[0] $ARGV[1].aligned.discordant.bed > tmp");
	my $line1="";
	my $line2="";
...
open (input, "<$ARGV[1].reference1.fa") or die "Can't open $ARGV[1].breakpoints.fa since $!\n";
while (my $line=<input>) {
    chomp($line);
    if ($line =~ /^>/) {
        $line =~ s/\>//;
...
close input;
open (input, "<$ARGV[1].reference2.fa") or die "Can't open $ARGV[1].breakpoints.fa since $!\n";
while (my $line=<input>) {
    chomp($line);
    if ($line =~ /^>/) {
        $line =~ s/\>//;
...
system("Rscript $ARGV[2]/dist_mean_sd.R");
...
open (output, ">>$ARGV[0]\_in_$ARGV[1].breakpoints.freq") or die "Can't open $ARGV[0].breakpoints.freq since $!\n";
...
#    if (($type{$bp} eq "O") && ($z[1] eq $z[3]) && (abs($z[4]-$z[2])+$samp{$bp}-1000 <= 3*$dist_sd)) {next;}
    ...
    {
	if ((defined $me_samp{$id}) && ($mm_samp{$id} <= 4) && (abs($dist_samp{$id}-$dist_mean)/$dist_sd <= 2.5)) {
	    my $pt2=$samp{$bp}-500;
    ...
    {
	if ((defined $me_ref1{$id}) && ($mm_ref1{$id} <= 6) && (abs($dist_ref1{$id}-$dist_mean)/$dist_sd <= 2.5)) {
    ...
    {
        if ((defined $me_ref2{$id}) && ($mm_ref2{$id} <= 6) && (abs($dist_ref2{$id}-$dist_mean)/$dist_sd <= 2.5)) {
	...
	system("Rscript $ARGV[2]/decide_reads.R $dist_mean $dist_sd");
